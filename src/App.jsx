...
const starterDates = [
  ...
  },
  {
    ...
    places: [
      ...
      { name: "The Violet Hour", type: "Drinks", ordered: ["Custom cocktails"] },
    ],
  ...
  },
  {
    ...
    rating: 8.7,
    places: [
      { name: "Art Institute", type: "Activity", ordered: [] },
...
const allNeighborhoods = ["West Loop", "River North", "Loop", "Logan Square", "Wicker Park", "Lincoln Park", "Lakeview", "Gold Coast", "Hyde Park", "Andersonville", "Chinatown", "Other"];
const allPrices = ["$", "$$", "$$$", "$$$$"];
      ...
      <main className="mx-auto max-w-7xl p-6">
        <section className="mb-6 rounded-[2rem] bg-white p-6 shadow-sm ring-1 ring-slate-200">
          <div className="flex flex-col justify-between gap-4 md:flex-row md:items-start">
            <div>
              <p className="text-sm font-bold uppercase tracking-[0.2em] text-orange-500">Plan a date</p>
              <h2 className="mt-2 text-3xl font-black">Find a date night that matches the vibe</h2>
              <p className="mt-2 text-sm text-slate-500">Filter your saved dates by activity, neighborhood, and vibe to get matching options.</p>
            </div>
            <button onClick={resetDemoData} className="rounded-2xl bg-slate-100 px-4 py-3 text-sm font-black text-slate-600 hover:bg-slate-200">
              Reset demo data
            </button>
          </div>
        ...
        {filtered.length === 0 ? (
          <div className="rounded-[2rem] bg-white p-10 text-center shadow-sm ring-1 ring-slate-200">
            ...
            <p className="mt-2 text-slate-500">Try different filters or add a new date with those attributes.</p>
          </div>
        ) : (
          <div className="grid gap-6 lg:grid-cols-2">
            {filtered.map((date) => <DateCard key={date.id} date={date} onDelete={deleteDate} />)}
          </div>
        )}
      </main>
          ...
          <form onSubmit={submitDate} className="mx-auto my-8 max-w-4xl rounded-[2rem] bg-white p-6 shadow-2xl">
            <div className="mb-6 flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">This saves locally in this browser for now. Supabase can sync it across devices later.</p>
              </div>
            ...
            <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:justify-end">
              ...
              <button type="submit" className="rounded-2xl bg-orange-500 px-6 py-3 font-black text-white shadow-lg shadow-orange-200 hover:bg-orange-600">Add date to list</button>
            </div>
          </form>
        </div>
      )}
